...
const SearchResult = () => {
  ...
  const fetchSearchResults = () => {
    let playload = { q: query, start: startIndex };
    if (imageSearch) {
      playload.searchType = "image";
    }
    fetchDataFromApi(playload).then((res) => {
      console.log(res);
      setresult(res);
    });
  };
    ...
    <div className="flex flex-col min-h-[100vh]">
      ...
      <main className="grow p-[12px] pb-0 md:pr-5 md:pl-20">
        ...
        {!imageSearch ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-col-6 gap-4">
            {items.map((item, index) => (
